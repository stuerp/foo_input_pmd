...
uint8_t * PMD::ExecuteADPCMCommand(Channel * channel, uint8_t * si)
{
    ...
    switch (al)
    {
        ...
        case 0xec: si = SetADPCMPanCommand(channel, si); break;        // FOR SB2
...
uint8_t * PMD::SetADPCMPortamento(Channel * channel, uint8_t * si)
{
    ...
    int al_ = (int) channel->onkai;

    fnumsetm(channel, oshift(channel, *si++));
    ...
    if (*si == 0xfb)
        channel->keyoff_flag = 2; // If '&' is immediately followed, KeyOff is not performed.
